...
cv::Mat gray_img, src_img,src_img1;
...
void on_mouse(int event, int x, int y, int flags, void* ustc)
{
    ...
    {
        cv::Point pre_pt = cv::Point(x, y);
	char temp[16];
        std::sprintf(temp,"x:%d,y:%d",x,y);
        cv::putText(src_img1,temp,cv::Point(x,y),CV_FONT_HERSHEY_SIMPLEX,0.5,cv::Scalar(150,55,245));
	cv::circle( src_img1, cv::Point(x,y), 2,cvScalar(255,0,0,0) ,CV_FILLED, CV_AA, 0 );
...
int main(int argc, char **argv) {
    ...
    cv::resize(src_img,src_img1,cv::Size(src_img.cols/2,src_img.rows/2));
    ...
    cv::setMouseCallback("source",on_mouse,0);
    cv::imshow("source", src_img1);
    cv::waitKey(0);
    ...
    cv::cvtColor(src_img1, gray_img, cv::COLOR_BGR2GRAY);
    cv::cornerSubPix(gray_img, corners, winSize, zerozone, criteria);

    cv::Mat result_img = src_img1.clone();
    for(uint t = 0; t < corners.size(); ++t) {
        ...
        writeData(output_name, corners[t].x*2, corners[t].y*2, t);
    }
